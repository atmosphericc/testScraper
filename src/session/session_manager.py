...
class SessionManager:
    ...
    async def _validate_session(self, attempt_recovery: bool = True, skip_initial_navigation: bool = False) -> bool:
        ...
        try:
            ...
            # Navigate to Target homepage to test session (skip if we just navigated during init)
            if not skip_initial_navigation:
                ...
                try:
                    ...
                    self.logger.warning(f"Validation navigation with 'commit' failed, trying 'load': {goto_error}")
                    await page.goto("https://www.target.com",
                                  wait_until='load',
                                  timeout=self.validation_timeout)
            else:
                self.logger.info("[NAV] [VALIDATION] Skipping navigation - already on Target.com from initialization")

            current_url = page.url
            self.logger.info(f"📍 [VALIDATION] Current URL: {current_url}")

            # Minimal wait for login state to load - page should already be stable
            self.logger.info("⏳ [VALIDATION] Waiting 0.3s for page to stabilize...")
            ...
            if not login_found:
                ...
                self.logger.error(f"[ERROR] [VALIDATION] Current URL: {current_url}")
                self.logger.error("[ERROR] [VALIDATION] This suggests session cookies may have been lost between restarts")
    ...
    async def _auto_login(self, email: str, password: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                # Fallback: Try with load if commit fails
                self.logger.warning(f"Login navigation with 'commit' failed, trying 'load': {goto_error}")
                await page.goto("https://www.target.com/login",
                              wait_until='load',
                              timeout=30000)
