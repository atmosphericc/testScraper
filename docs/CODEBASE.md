# Codebase Map
## Last Updated: 2026-05-06 PM (Phase 4c LIVE-VALIDATED — `TARGET_API_CART_CLEAR=true` exercised over 5 consecutive TEST_MODE cycles on TCIN 50270379. Each cycle: GET /cart returned 1 cart_item, DELETE /cart_items/<id> returned 200 first try, SFL pass continued on DOM. Zero fallbacks to DOM clear, zero `[CLEAR_CART_API] exception` lines. Cycle timings 12.79s–19.01s end-to-end; cart-clear itself is sub-second. Phase 4c is now production-ready.)
Previous: 2026-05-06 (Phase 4b IMPLEMENTED — `_api_place_order` written + wired into `_place_order` behind `TARGET_API_PLACE_ORDER=true`. API fires first; falls back to DOM on Shape/auth failures; bails on terminal OOS/RESERVATION rejections. `TARGET_API_PLACE_ORDER_OBSERVE=true` logs the success-response body to `logs/api_capture.log` so order_id parsing can be validated on first real-order run. `_api_order_id` / `_api_confirmation_url` plumbed through `_complete_checkout`'s success dict — DOM mode unchanged.)
...
- [x] Walmart hybrid checkout API research — `docs/RETAILERS/WALMART_CHECKOUT_API.md`
  - Researched: 2026-05-11. REST API at `/api/checkout/v3/` confirmed (8-step sequence). Key finding: Python HTTP hybrid is NOT viable (PerimeterX is TLS-session-bound); viable approach is `tab.evaluate(fetch(...))` from inside the real Chrome tab. `orderLimit` field confirmed at `props.pageProps.initialData.data.product.orderLimit`. Queue uses Queue-it (`queue-it.net`); FBT bypass confirmed as primary queue bypass for Pokemon/hot drops. PIE encryption required for CVV tokenization. Estimated hybrid speed: 2.5–4s end-to-end. Flagged for @purchase-flow-engineer.
