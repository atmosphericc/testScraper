...
**Fix Applied**: 2026-09-25 post-run, all in the wrapper: K1 `TARGET_WAVE_FIRST_EDGE=1` (A1 killed by its pre-registered rule, verified), R1 `TARGET_WONCART_RF_ENTRY=1` (new code: a 429 RESERVATION_FAILURE enters the ticket loop; 424 stays out; verified mechanism, no new double-order path), L1 `RESILIENT_206_LOG=1` (new code, log-only: the 206 body summary nobody has ever seen), L2 `TARGET_TOKEN_MINT_LOG=1` (new code, log-only: Target's answer to the mint). Not done on purpose: a 206 ingest (the parser reads missing fulfillment as out of stock — needs the L1 shape first), a token keep-and-restore fix (its motivating claim was REFUTED; it buys ≤30 min), the DOM-detour skip and the legacy-hold OOS stop (R1 routes the observed case around both). Tests: `tests/test_redsky_206_log.py` (28), `tests/test_token_mint_log.py` (22), 11 new checks in `tests/test_won_cart_direct_smoke.py`; every new branch mutation-checked. Readout: `tools/analysis/readout_arm_2026_09_25.py`.
**Confidence**: high on the funnel, the gate leak and the kill (all verified from fresh contexts, Opus 5.5); LOW that R1 converts a hot cart (2 recoveries after a 429 RF, both 08-04; 0 hot orders ever); cause of the 206 bursts and of the mint outage NOT ESTABLISHED.
**Outcome**: UNPROVEN LIVE. Before the next run: hand-login business (and check all three), then judge with the pre-registered readout before any narrative.
**Addendum (first-gate investigation, same day; C-0925-07..11)**: only first shots pass the edge — flip-race first shots 15/142 (home IP 15/65) vs every other shot 14/4,013 over six restock nights, so ~97% of first-gate failures are re-shots and re-flips. Across the bot's life 9 hot carts were won and 0 converted: 4 never reached a place-order, 5 died on the checkout FAST_SELLING throttle — the case R5's 5 s ticket loop was built for and has never run on (R1 now routes the 09-25 case into it). qty 2 is NOT the cause (REFUTED); firing before the flip has no support. Armed: L3 `RESILIENT_FLIP_LOG=1` (log-only: an epoch-ms stamp on the in-stock read plus RedSky's raw ATP / limit fields; `tests/test_redsky_flip_log.py`, readout L3).

### [2026-09-23] - 0-for on the 30th Celebration restock: 5 windows, 37 shots, all 429 — wave-first allowed 4 shots per account per window, alt-1 sat idle behind the cap, 8 shots fired into gone stock; a latent same-TCIN double-race path found in the process - TARGET
**Symptom**: `run_20260922_232734.log` (launched 09-22 23:27). Five restock windows on four hot TCINs — 1010892076 at 02:47 and 03:37, 1010892067 at 04:03, 1010892069 at 04:41, 1010892078 at 05:16 (7 in-stock flickers in 42 s) — each ~1-2.5 min long. 9 races, 37 main-tab add-to-cart POSTs, every one HTTP 429: 36 `ERR_A2C_TCIN_RATE_LIMITED` (no Envoy timing — rejected at the edge), 1 `FAST_SELLING_ITEM_RATE_LIMIT_EXCEPTION` (04:41:06, primary, reached the cart service; the DCO burst re-POST 1.6 s later drew an edge 429). 0 carts, 0 orders. Detection to first POST ≤1 s every time; monitor never blind; no 401, no 403, no challenge.
**Root Cause**: Not a bot fault and not a regime change (C-0923-06): Target's per-TCIN high-demand limiter admitted 1 of 37, in line with the hot-SKU history. What was ours is how FEW tickets we bought: (1) `TARGET_WAVE_FIRST_EDGE=1` put every edge 429 on a 55-70 s cold re-entry and the 83 s end-of-window floor ended the race after the second shot, so each account fired at t≈0, ~60 s, ~73 s (re-armed race) and ~135 s — nothing 3-55 s into any window (C-0923-01); (2) `TARGET_MULTI_SKU_WORKERS_PER_TCIN=2` kept alt-1 out of all 9 races although it was logged in and no second TCIN was ever live (C-0923-04); (3) the retry loop never reads stock, so 8 of the 37 shots went out 0.1-19 s after the monitor had already read the TCIN out of stock (C-0923-03). Whether faster re-shots would have passed more often cannot be settled from the logs (C-0923-02, NOT ESTABLISHED either way).
**Fix Applied**: all flag-gated, armed in the wrapper 2026-09-23 (uncommitted at time of writing): A1 `TARGET_WAVE_FIRST_EDGE=0` (edge and DCO 429s re-fire at the 2.0-3.0 s edge cadence; 401s keep the cold re-entry — a labelled BET with a pre-registered kill rule), A2 `TARGET_MULTI_SKU_WORKERS_PER_TCIN=3`, A3 `TARGET_RETRY_STOP_WHEN_OOS=1` + `TARGET_RETRY_OOS_STOP_S=8` (new code: end the window after 8 s of unbroken out-of-stock reads; fail-open), and G1 `TARGET_STUCK_RESET_LIVE_GUARD=1` (new code: a live racer is never treated as stuck — guarded at the SILENT 60 s reset in `reset_completed_purchases_by_stock_status`, which runs first in the production handler, at the 60 s reset in `process_stock_data`, at the dispatch point as a catch-all, and at the 120 s reservation-TTL sweep. Without it a race running past 60 s can be joined by a second race on the same TCIN that re-claims the same accounts, C-0923-05; the silent reset has already fired on live races 6 times in production, C-0923-08, harmless only because multi-SKU dispatch was off then. The first cut guarded only the second copy and an adversarial review refuted it the same day). A second adversarial review confirmed the same-TCIN protection and live-reproduced a pre-existing CROSS-TCIN gap (C-0923-09: with at most 1 ready account the legacy dispatch path takes no reservation, so a busy account could be handed a second TCIN), also closed under G1. Tests: `tests/test_retry_oos_stop.py` (112 checks, every guard point mutation-checked), pins updated in `tests/test_redsky_apps_channel.py` and `tests/test_0828_phase2_fixes.py`; offline suite 27/27. Readout: `tools/analysis/readout_arm_2026_09_23.py`.
**Confidence**: high on the mechanisms (all four verified from fresh contexts); LOW on the outcome — A1 is a bet on an unsettled question and hot SKUs have never converted in this bot's life (0 of 5-7 hot carts ever).
**Outcome**: UNPROVEN LIVE. Judge on the next restock with the pre-registered readout before any narrative.
...
[REFUTED 2026-09-25, C-0925-09: qty-2 carts converted after RESERVATION_FAILURE on sibling SKUs, and
1011483413 is classed ordinary; the one hot-list cart, 1011960739, was emptied at pre_checkout].
...
the cart" lines; tickets fired after an eviction (should be 0). The qty-1 readout NEVER RAN: the pin fired
no shot before the operator restored qty 2 on 09-22 (C-0925-09).
